...
Estamos confiados que como resultado do Teorema temos funções de densidade. Vejamos neste exemplo se isso é realmente acontece. Consideremos, para simplificar, o caso $n$ = 3 e verifiquemos se a integral da função de densidade em é 1. Então
...
f (x_{(1)}, x_{(2)}, x_{(3)}) dx_{(1)} dx_{(2)} dx_{(3)}	=  6
...
P (X_{(1)} < X_{(2)} < X_{(3)})  =  1 − P (X1 = X2 ̸= X3) − P (X1 = X3 ̸= X2)
−P (X2 = X3 ̸= X1) − P (X1 = X2 = X3)
a qual pode ser escrita como
P (X_{(1)} < X_{(2)} < X_{(3)}) = 1 − 3P (X1 = X2 ̸= X3) − P (X1 = X2 = X3)
...
n	n
P (X_{(1)} = X_{(2)} = \cdots = X_{(n)})  =  ∏ P (X(k) = 0) + ∏ P (X(k) = 1)
...
**Exemplo 2.8**
Sejam X1, X2, X3, X4 variáveis aleatórias independentes com densidade comum f.	A função de densidade conjunta das estatísticas de ordem X_{(1)}, X_{(2)}, X_{(3)}, X_{(4)} é
...
) =	4!f (x_{(1)})f (x_{(2)})f (x_{(3)})f (x_{(4)}),	se	x_{(1)} < x_{(2)} < x_{(3)} < x_{(4)}  ·
0,	caso contrário
 

Vamos calcular a função de densidade marginal de X_{(2)}. Temos que, se x_{(1)} < x_{(2)} < x_{(3)} < x_{(4)} f2(x_{(2)})  =  4! ∫ ∫ ∫ ∫ $F$ (t1)f (x_{(2)})f (t3)f (t4) dt1 dt3 dt4
